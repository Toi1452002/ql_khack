...
class HopdongData {
  ...
  Future<Response> getViewHopDong({int hieuLuc = 1, int thoiHan = 1, int dN = 0}) async {
    return _dio.get(PathServer.hopdong,
        queryParameters:
            PathServer.push(type: 'get-view-hopdong', data: {'HieuLuc': hieuLuc, 'ThoiHan': thoiHan, 'DN': dN}));
  }

  Future<Response> getHopDong() async {
    return _dio.get(PathServer.hopdong, queryParameters: PathServer.push(type: 'get-hopdong'));
  }

  Future<Response> getDSMaKichHoat(Map<String, dynamic> data) async {
    return _dio.get(PathServer.hopdong, queryParameters: PathServer.push(type: 'get-ma-kich-hoat', data: data));
  }
